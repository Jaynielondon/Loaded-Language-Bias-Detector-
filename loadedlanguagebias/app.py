import reimport htmlimport requestsimport streamlit as stfrom bs4 import BeautifulSoupfrom datetime import datetime
...
This app analyzes article text, headline language, source transparency,

loaded wording, tone, framing, and bias categories.

It is designed as a Python web app that can be hosted online.
...
st.set_page_config(page_title="BiasLens | Media Bias Analysis Tool",page_icon="📰",layout="wide")
...
st.markdown(""".stApp {background: #f7f7f4;color: #1f1f1f;}
...
bias_patterns = {"Emotional Language": {"patterns": [r"\boutrageous\b", r"\bshocking\b", r"\bterrifying\b", r"\bhorrific\b",r"\bdisgusting\b", r"\bheartbreaking\b", r"\bdevastating\b",r"\bdangerous\b", r"\btragic\b", r"\bfurious\b", r"\balarming\b",r"\bscary\b", r"\bpanic\b", r"\bevil\b", r"\bchaos\b", r"\bcrisis\b"],"explanation": "This wording may create a strong emotional reaction instead of staying neutral.","css": "highlight-emotional"},"Extreme Language": {"patterns": [r"\balways\b", r"\bnever\b", r"\beveryone\b", r"\bno one\b",r"\bcompletely\b", r"\btotally\b", r"\babsolutely\b", r"\bentirely\b",r"\bworst\b", r"\bbest\b", r"\ball\b", r"\bnothing\b",r"\bmust\b", r"\bguaranteed\b", r"\bundeniable\b"],"explanation": "Extreme wording can make a claim sound more certain or dramatic than the evidence may support.","css": "highlight-extreme"},"Loaded Wording": {"patterns": [r"\bpropaganda\b", r"\bcorrupt\b", r"\bradical\b", r"\bagenda\b",r"\bbrainwashed\b", r"\bfake\b", r"\blies\b", r"\bmanipulated\b",r"\belites\b", r"\bdisaster\b", r"\bfailure\b", r"\bincompetent\b",r"\bcriminal\b", r"\breckless\b", r"\battack\b", r"\bdestroy\b"],"explanation": "Loaded terms can push the reader toward a judgment before giving full evidence.","css": "highlight-loaded"},"One-Sided Framing": {"patterns": [r"critics say", r"supporters insist", r"experts warn", r"many believe",r"some people claim", r"it is clear that", r"the truth is",r"without question", r"there is no doubt", r"refused to answer"],"explanation": "This phrase may frame the issue from one side or present a claim as obvious.","css": "highlight-framing"},"Opinionated Phrasing": {"patterns": [r"\bshould\b", r"\bclearly\b", r"\bobviously\b", r"\bwrong\b",r"\bright\b", r"\bunfair\b", r"\bdeserves\b", r"\bfailed to\b",r"\brefused to\b", r"\bfinally admitted\b"],"explanation": "This wording may suggest the writer is interpreting or judging the event instead of only reporting it.","css": "highlight-opinion"}}

factual_markers = ["according to", "reported", "data", "study", "survey", "evidence","percent", "%", "said", "stated", "announced", "published","research", "official", "records show", "documents show"]

balance_markers = ["however", "although", "on the other hand", "meanwhile", "but","supporters", "critics", "opponents", "different views", "both sides"]

quote_markers = ["said", "stated", "told", "according to", "wrote", "reported", "explained", "announced"]data_markers = ["data", "study", "survey", "research", "percent", "%", "statistics", "report", "records"]anonymous_markers = ["anonymous", "unnamed source", "sources familiar", "person familiar", "officials familiar"]
...
def split_into_sentences(text):sentences = re.split(r"(?<=[.!?])\s+", text.strip())return [sentence.strip() for sentence in sentences if sentence.strip()]

def get_words(text):return re.findall(r"\b[\w'-]+\b", text.lower())

def count_marker_matches(text, markers):lowered = text.lower()return sum(lowered.count(marker.lower()) for marker in markers)

def detect_tone(sentence):lowered = sentence.lower()
...
def get_score_description(score):if score < 15:return "Low: The article has a mostly neutral wording pattern based on this prototype."if score < 35:return "Moderate: Some wording may influence the reader, but the article is not heavily loaded overall."if score < 60:return "High: The article contains several loaded, emotional, or opinionated language patterns."return "Very High: The article uses a lot of language that may strongly shape the reader's opinion."

def find_bias_in_sentence(sentence):sentence_results = []lowered_sentence = sentence.lower()
...
def calculate_bias_score(text, results):sentences = split_into_sentences(text)words = get_words(text)
...
def analyze_text(text):text = clean_text(text)sentences = split_into_sentences(text)all_results = []
...
def get_category_counts(results):counts = {}for item in results:counts[item["category"]] = counts.get(item["category"], 0) + 1return counts

def get_top_category(results):counts = get_category_counts(results)if not counts:return "None"return max(counts, key=counts.get)
...
def fetch_article_from_url(url):headers = {"User-Agent": "Mozilla/5.0"}response = requests.get(url, headers=headers, timeout=10)response.raise_for_status()
...
def build_highlighted_html(text, results):if not text:return ""
...
def compare_headline_to_article(headline, article_text):headline = clean_text(headline)article_text = clean_text(article_text)
...
def calculate_transparency_score(article_text, headline, user_checks):text = clean_text(article_text)lowered = text.lower()
...
def create_report(headline, article_text, score, results, headline_analysis=None, transparency=None):category_counts = get_category_counts(results)flagged_sentence_count = len(set(item["sentence"] for item in results))total_sentences = len(split_into_sentences(article_text))
...
st.markdown('BiasLens', unsafe_allow_html=True)st.markdown('A clean Python-based media bias analysis tool that reviews loaded language, emotional tone, framing, headline strength, and source transparency.',unsafe_allow_html=True)

with st.sidebar:st.subheader("How to use")st.write("1. Paste an article URL or article text.")st.write("2. Add a headline if you want headline analysis.")st.write("3. Click Analyze Article.")st.write("4. Review highlighted wording, score, and explanations.")st.divider()st.caption("This is a prototype. It helps users review media language, but it should not be treated as a final fact-checker.")

left_col, right_col = st.columns([1.1, 0.9], gap="large")

with left_col:st.markdown('', unsafe_allow_html=True)st.markdown('Input', unsafe_allow_html=True)
...
with right_col:st.markdown('', unsafe_allow_html=True)st.markdown('Optional Advanced Checks', unsafe_allow_html=True)
...
if analyze_clicked:if not article_text.strip():st.error("Please enter article text or load an article from a URL first.")else:score, results = analyze_text(article_text)category_counts = get_category_counts(results)total_sentences = len(split_into_sentences(article_text))flagged_sentence_count = len(set(item["sentence"] for item in results))top_category = get_top_category(results)
